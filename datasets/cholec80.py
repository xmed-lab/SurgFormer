...
class CholecDataset(Dataset):
    def __init__(self, file_paths, file_labels, transform, train_num_each_40, sequence_length, loader=pil_loader):
        self.file_paths = file_paths  # all the samples 98234
        self.file_labels_phase = file_labels[:, 0]
        self.file_labels_tool = file_labels[:, 1: 8]
        self.file_labels_phase_ant = file_labels[:, 8: 15]
        self.file_labels_tool_ant = file_labels[:, 15: 22]
        self.transform = transform
        self.loader = loader
        self.train_num_each = train_num_each_40
        self.sequence_length = sequence_length
        self.phase_classes = 7

        self.prepare_metas(train_num_each_40, sequence_length)
    ...
    def __getitem__(self, index):
        ...
        target = {
            'labels_phase': labels_phases,  # Tx7
            'labels_tool': labels_tools,    # Tx7
            'labels_phase_ant': labels_phase_ants,  #Tx7
            'labels_tool_ant': labels_tool_ants  # Tx7
        }
        return imgs, target
...
def build(image_set, args):
    PATHS = {
        "train": ('/home/eemenglan/datasets/cholec80/cholec80_train_paths_labels1_4task_40.pkl'),
    ...
    train_paths_40 = train_paths_labels[0]
    train_labels_40 = train_paths_labels[1]
    train_num_each_40 = train_paths_labels[2]

    # print('train_paths_28  : {:6d}'.format(len(train_paths_28)))
    # print('train_labels_28 : {:6d}'.format(len(train_labels_28)))


    train_labels_40 = np.asarray(train_labels_40, dtype=np.float32)
    ...
    train_dataset_40 = CholecDataset(train_paths_40, train_labels_40, train_transforms, train_num_each_40, args.sequence_length)


    return train_dataset_40 #, train_num_each_28
